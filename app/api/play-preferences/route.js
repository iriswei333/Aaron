...
export async function PUT(request) {
  try {
    ...
    const current = await updateCurrentProfileField(request, 'playPreferences', body);
...
export async function GET(request) {
  try {
    ...
    if (!current.user) return profileErrorResponse(current);
    return Response.json({ playPreferences: current.user.playPreferences, authMode: current.mode });
  } catch (error) {
    return Response.json({ error: error.message }, { status: 500 });
  }
}
