...
export function defaultStore() {
  return { users: {}, posts: [], playDates: [], familyEventCache: {}, playgroundCache: {}, parentingResourceCache: {}, familyEvents: [], chatMessages: [] };
}
...
function normalizeStore(store) {
  ...
  return {
    ...
    familyEvents: Array.isArray(store?.familyEvents)
      ? store.familyEvents.filter((item) => item.kind === 'external_event')
      : [],
...
export async function listLocalChatThreads(userId) {
  ...
  store.playDates.forEach((playDate) => {
    ...
    threads.push({ id: localPlaydateThreadId(playDate.id), type: 'playdate', playDateId: playDate.id, title: playDate.playgroundName || 'Playdate chat', participantIds: joined });
  });
  ...
  [...related].forEach((id) => {
    ...
    if (!user) return;
    threads.push({ id: localDirectThreadId(userId, id), type: 'direct', title: user.displayName || 'Parent', participantIds: [userId, id], contactId: id, participants: [{ id, displayName: user.displayName || 'Parent' }] });
  });
  return threads;
}
...
export async function listSupabaseChatThreads(supabase, authUser) {
  const { data: memberships, error: membershipError } = await supabase.from('chat_thread_members').select('thread_id').eq('user_id', authUser.id);
  ...
  const { data: playDates, error: playDateError } = playDateIds.length ? await supabase.from('play_dates').select('id, playground_name, starts_at').in('id', playDateIds) : { data: [], error: null };
  ...
  return (threadRows || []).map((row) => {
    ...
    return { id: row.id, type: row.thread_type, playDateId: row.play_date_id, title: playDate?.playground_name || other[0]?.display_name || 'Direct chat', participantIds, participants: other.map((profile) => ({ id: profile.id, displayName: profile.display_name || 'Parent' })), startsAt: playDate?.starts_at || null };
  });
}
