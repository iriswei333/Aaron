...
export function defaultStore() {
  return { users: {}, posts: [], playDates: [], familyEventCache: {}, playgroundCache: {}, chatMessages: [] };
}
...
export async function listSupabaseChatContacts(supabase, authUser) {
  ...
  if (otherError || !others?.length) return [];
  ...
  return (profiles || []).map((profile) => ({ id: profile.id, displayName: profile.display_name || 'Parent', email: profile.email || '', childProfile: normalizeChildProfile(profile.child_profile) }));
}
