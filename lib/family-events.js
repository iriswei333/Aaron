...
async function fetchParentMapEvents({ locationRegion, startDate, endDate, childProfile, page = 1 }) {
  if (!locationRegion) return { events: [], sourceUrls: [], errors: [] };

  const ageSlugs = ageSlugsForChild(childProfile);
  const requestedUrls = datesBetween(startDate, endDate).map((date) => parentMapUrl({ date, region: locationRegion, ageSlugs, page }));
  const dayRequests = requestedUrls.map(async (url, index) => {
    const date = datesBetween(startDate, endDate)[index];
...
export async function fetchFamilyEvents({ locationCity, startDate, endDate, childProfile = {}, page = 1 }) {
