...
export function parseParentMapEventWebsite(html, eventUrl) {
  for (const anchor of String(html || '').matchAll(/<a\b([^>]*)>([\s\S]*?)<\/a>/gi)) {
    if (htmlToText(anchor[2]).toLowerCase() !== 'event website') continue;
    const href = anchor[1].match(/\bhref\s*=\s*(["'])(.*?)\1/i)?.[2];
    if (!href?.trim()) continue;
    try {
      const url = new URL(decodeHtml(href).trim(), eventUrl);
      if (['http:', 'https:'].includes(url.protocol)) return url.href;
    } catch {
      // Keep the listing link when the website button has an invalid URL.
    }
  }
  return '';
}

export async function resolveFamilyEventWebsites(events) {
  const requests = new Map();
  return Promise.all(events.map(async (event) => {
    if (event.source !== 'parentmap' || event.resultType !== 'event'
      || !/^https?:\/\/www\.parentmap\.com\/calendar\//i.test(String(event.url || ''))) return event;
    if (!requests.has(event.url)) {
      requests.set(event.url, fetchTextWithTimeout(event.url)
        .then((html) => parseParentMapEventWebsite(html, event.url))
        .catch(() => ''));
    }
    const websiteUrl = await requests.get(event.url);
    return websiteUrl ? { ...event, listingUrl: event.url, url: websiteUrl } : event;
  }));
}
