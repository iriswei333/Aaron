export const DEFAULT_PLAYGROUND_RADIUS_METERS = 4828;
...
async function fetchGoogleNearbyPlaygrounds({ latitude, longitude, radiusMeters = DEFAULT_PLAYGROUND_RADIUS_METERS }) {
...
function overpassQuery(latitude, longitude, radiusMeters = DEFAULT_PLAYGROUND_RADIUS_METERS) {
  const selectors = [
    '["leisure"="playground"]',
    '["leisure"="park"]',
    '["amenity"="library"]',
  ...
  const clauses = selectors.flatMap((selector) => [
    `node${selector}(around:${radiusMeters},${latitude},${longitude});`,
    `way${selector}(around:${radiusMeters},${latitude},${longitude});`,
    `relation${selector}(around:${radiusMeters},${latitude},${longitude});`,
  ]).join('\n');
  return `[out:json][timeout:12];(${clauses});out center tags 40;`;
}

export function playgroundCacheKey({ latitude, longitude, radiusMeters = DEFAULT_PLAYGROUND_RADIUS_METERS }) {
  return `nearby-v5-${Number(latitude).toFixed(3)}-${Number(longitude).toFixed(3)}-${Math.round(radiusMeters)}`;
}

async function fetchNearbyPlaygroundsFromOverpass({ latitude, longitude, radiusMeters = DEFAULT_PLAYGROUND_RADIUS_METERS }) {
