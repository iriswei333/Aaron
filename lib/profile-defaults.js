...
export function normalizeChild(input = {}, previous = {}) {
  ...
  return {
    ...
    diaperSize: Number.isFinite(Number(merged.diaperSize)) && Number(merged.diaperSize) >= 1 && Number(merged.diaperSize) <= 8 ? Number(merged.diaperSize) : null,
    feedingStage: cleanText(merged.feedingStage || merged.feeding_stage, 80),
    daycareDays: cleanList(merged.daycareDays || merged.daycare_days, 7),
