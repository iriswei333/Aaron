...
export async function updateCurrentProfileDetails(request, body = {}) {
  ...
  if (current.mode === 'supabase') {
    ...
    const { data, error } = await current.supabase
      ...
      .select('id, email, display_name, child_profile, social_links, location, food_plan, amazon_errands, family_objects, created_at, updated_at')
      .single();
    ...
    return {
      ...
      user: {
        ...
        amazonErrands: data.amazon_errands || { tasks: [], outfitIdeas: [], reminder: null },
        familyObjects: Array.isArray(data.family_objects) ? data.family_objects : [],
      },
    };
  }
