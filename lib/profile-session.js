...
import { getChildProfile, normalizeChildProfile, normalizePlayPreferences } from './profile-defaults.js';
...
export async function getCurrentProfile(request, options = {}) {
  ...
  const user = storedUser
    ? {
      ...
      playPreferences: normalizePlayPreferences(storedUser.playPreferences),
    }
    : null;
  if (!user) {
    return { mode: 'local', status: 401, error: 'Choose a local family profile.' };
  ...
  if (current.mode === 'supabase') {
    ...
    const { data, error } = await current.supabase
      ...
      .eq('id', current.authUser.id)
        .select('id, email, display_name, child_profile, social_links, location, play_preferences, created_at, updated_at')
      .single();
    ...
    return {
      ...
      user: {
        ...
        playPreferences: normalizePlayPreferences(data.play_preferences),
      },
    };
  }
...
export async function updateCurrentProfileField(request, field, body) {
  if (!['location', 'socialLinks', 'childProfile', 'playPreferences'].includes(field)) {
    ...
    const value = field === 'location'
      ...
      : field === 'childProfile'
        ...
        : field === 'playPreferences'
          ? normalizePlayPreferences({ ...target.playPreferences, ...body })
        : { ...target[field], ...body };
