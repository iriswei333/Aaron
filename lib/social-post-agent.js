...
function bestEvent(events = [], context = {}) {
  return events
    ...
    .sort((a, b) => eventScore(b, context) - eventScore(a, context))[0] || null;
}
...
export function makeWeeklyRoundup(posts = [], startDate = '', endDate = '') {
  ...
  const lines = posts.map((post, index) => {
    const details = [post.venue].filter(Boolean).join(' · ');
    const highlights = (post.highlights || []).slice(0, 1).map((highlight) => `亮点：${firstSentence(highlight)}`).join('；');
    const keywords = (post.matchingKeywords || []).length ? `趋势关键词：${post.matchingKeywords.join('、')}\n   ` : '';
  ...
  return {
    ...
    posts: posts.map((post) => ({ city: post.city, title: post.title, date: post.date, dateLabel: post.dateLabel, timeLabel: post.timeLabel, venue: post.venue, highlights: post.highlights, matchingKeywords: post.matchingKeywords, recommendationReasons: post.recommendationReasons, sourceLabel: post.sourceLabel, eventUrl: post.eventUrl })),
    startDate,
    endDate,
    author: 'SproutCue',
  };
...
export async function generateWeeklySocialPosts({
  ...
  const results = await Promise.all(regions.flatMap((region) => days.map(async (day) => {
    ...
    try {
      ...
      const events = [...forcedEvents, ...(fetched.events || [])];
      ...
      if (matchedEvent) return { region, day, fetched: { ...fetched, sourceUrls }, events, event: matchedEvent, method: matchedEvent.sourceLabel || 'Family event source' };
      try {
  ...
  const selectedResults = results.map((result) => {
    ...
    const availableCandidates = candidates
      ...
      .sort((a, b) => eventScore(b, { city: clean(result.region.city || result.region.label), date: result.day })
        - eventScore(a, { city: clean(result.region.city || result.region.label), date: result.day }));
