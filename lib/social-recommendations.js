...
export const WEEKEND_EVENT_RECOMMENDATIONS = [
  ...
  {
    keyword: 'story time',
    terms: ['story time', 'storytime', 'story stroll', 'storywalk', 'toddler story', 'library story'],
    boost: 12,
    ageBoost: 6,
    ageTerms: ['toddler', 'toddlers', 'preschool', 'preschoolers', 'all-ages', 'all ages'],
    reason: 'welcoming early-learning topic for young children',
  },
];
...
export function recommendWeekendEvent(event, { city = '', date = '' } = {}) {
  ...
  for (const recommendation of WEEKEND_EVENT_RECOMMENDATIONS) {
    ...
    if (recommendation.ageBoost) {
      const audienceText = [event.ageSlugs, event.ageRange, event.ageLabel, event.audience]
        .flat(Infinity)
        .filter(Boolean)
        .join(' ')
        .toLowerCase();
      if (recommendation.ageTerms.some((term) => audienceText.includes(term))) {
        score += recommendation.ageBoost;
        reasons.push('age fit for toddlers and preschoolers');
      }
    }
