...
function renderPlayStudioShortcut(childName) {
  return `<section class="today-studio-shortcut" aria-labelledby="today-studio-title"><div class="today-studio-icon" aria-hidden="true">✦</div><div><p class="eyebrow">Play Studio</p><h2 id="today-studio-title">Make ${escapeHtml(childName)}’s first career book</h2><p>Turn a few favorite photos into a private bilingual book, one carefully reviewed page at a time.</p></div><button type="button" data-home-tab="studio">Open Play Studio <span aria-hidden="true">→</span></button></section>`;
}
